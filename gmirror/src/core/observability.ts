...
export class LocalAuditLogger {
  ...
  constructor(private tool: string) {
    this.auditDir = process.env.GMIRROR_AUDIT_DIR || path.join(os.homedir(), `.${tool}`, 'audit');
    this.retentionDays = Number(process.env.GMIRROR_LOG_RETENTION_DAYS || '7');
...
export class MetricsRegistry {
  ...
  recordPublicMethod(method: string, durationMs: number, status: 'ok' | 'error'): void {
    this.increment('gmirror_public_method_throughput_total', { method, status });
    this.observe('gmirror_public_method_latency_ms', durationMs, { method });
    if (status === 'error') this.increment('gmirror_public_method_errors_total', { method });
  }

  prometheus(): string {
    const lines: string[] = [
      '# HELP gmirror_public_method_throughput_total Total public method calls by method and status.',
      '# TYPE gmirror_public_method_throughput_total counter',
    ];
    for (const [key, value] of this.counters) {
      lines.push(`${key} ${value}`);
    }
    lines.push('# HELP gmirror_public_method_latency_ms Public method latency quantiles.');
    lines.push('# TYPE gmirror_public_method_latency_ms summary');
  ...
  openTelemetry(): Record<string, unknown> {
    return {
      resourceMetrics: [{
        resource: { attributes: [{ key: 'service.name', value: { stringValue: 'gmirror' } }] },
        scopeMetrics: [{
          scope: { name: 'gmirror.local' },
...
export class GMirrorObservability {
  ...
  constructor(private tool = 'gmirror') {
    this.logger = new LocalLogger(tool, (process.env.GMIRROR_LOG_LEVEL as LogLevel) || 'INFO');
  ...
  async alertOnHealthDrop(score: number, checks: HealthCheckLike[]): Promise<void> {
    const threshold = Number(process.env.GMIRROR_HEALTH_ALERT_THRESHOLD || '80');
    const webhook = process.env.GMIRROR_HEALTH_WEBHOOK_URL;
