/**
 * PostgreSQL engine implementation for concurrent GMirror deployments.
 */
...
export class PostgreSQLEngine implements BrainEngine {
  ...
  async initialize(): Promise<void> {
    const connectionString = this.config.connectionString || process.env.GMIRROR_POSTGRES_URL || process.env.DATABASE_URL;
    if (!connectionString) {
    ...
    const readConnectionString = this.config.readConnectionString || process.env.GMIRROR_POSTGRES_READ_REPLICA_URL;
