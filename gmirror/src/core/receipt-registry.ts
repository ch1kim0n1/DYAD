...
export class ReceiptRegistry {
  ...
  private async initializePersistence(): Promise<void> {
    try {
      ...
      throw new Error(`Persistence initialization failed: ${error}. Persistence is REQUIRED for GMirror.`);
    }
  }
  ...
  private async initializeSchema(): Promise<void> {
    try {
      ...
      if (existingSchema && existingSchema.version !== this.SCHEMA_VERSION) {
        logger.warn('Receipt schema version mismatch', {
    ...
    try {
      ...
      if (receipt._signature && this.signatureKey) {
        ...
        if (!verifyReceipt(data, _signature, this.signatureKey)) {
          logger.warn('Last receipt signature verification failed');
        }
      }
      ...
      for (const file of files) {
        ...
        for (const line of lines) {
          ...
          if ((receipt as any)._signature && this.signatureKey) {
            ...
            if (!verifyReceipt(data, _signature, this.signatureKey)) {
              logger.warn('Receipt signature verification failed', { timestamp: receipt.timestamp });
            }
          }
  ...
  private async pushDurable(receipt: any): Promise<void> {
    const storeUrl =
      process.env.GMIRROR_RECEIPT_S3_URL ||
      process.env.GMIRROR_RECEIPT_STORE_URL ||
      process.env.RECEIPT_STORE_URL;
    const storePath = process.env.GMIRROR_RECEIPT_STORE_PATH || process.env.RECEIPT_STORE_PATH;
