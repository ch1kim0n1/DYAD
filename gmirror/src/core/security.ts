...
const SECRET_ENV = Object.fromEntries([
  ...
  ['gmirror_auth_secret', ['GMIRROR', 'AUTH', 'SECRET'].join('_')],
  ['gmirror_mcp_token', ['GMIRROR', 'MCP', 'TOKEN'].join('_')],
  ['health_shutdown_token', ['GMIRROR', 'HEALTH', 'SHUTDOWN', 'TOKEN'].join('_')],
]) as Record<string, string>;

export class FileSecretManager {
  constructor(private rootDir = process.env.GMIRROR_SECRET_DIR || path.join(os.homedir(), '.gmirror', 'secrets')) {}
...
export class PermissionModel {
  ...
  static loadDefault(): PermissionModel {
    const file = process.env.GMIRROR_PERMISSIONS_FILE;
