...
describe('security controls', () => {
  it('rotates secrets without exposing values from list output', () => {
    const dir = mkdtempSync(join(tmpdir(), 'gmirror-secrets-'));
    try {
  ...
  it('sanitizes user-facing CLI inputs', () => {
    expect(sanitizeCliString('change', 'payload')).toBe('change');
    expect(() => sanitizeCliString('bad\0payload', 'payload')).toThrow('NUL');
  ...
  it('uses rate-limited health endpoints with token-protected shutdown', () => {
    ...
    expect(source).toContain('GMIRROR_HEALTH_RATE_LIMIT_RPM');
  ...
  it('loads permission grants from a JSON file without storing live tokens', () => {
    const dir = mkdtempSync(join(tmpdir(), 'gmirror-permissions-'));
    const previous = process.env.GMIRROR_PERMISSIONS_FILE;
    try {
      ...
      process.env.GMIRROR_PERMISSIONS_FILE = file;

      const permissions = PermissionModel.loadDefault();
      expect(permissions.scopesForToken(token, ['read', 'write'])).toEqual(['read']);
    } finally {
      if (previous === undefined) {
        delete process.env.GMIRROR_PERMISSIONS_FILE;
      } else {
        process.env.GMIRROR_PERMISSIONS_FILE = previous;
      }
      rmSync(dir, { recursive: true, force: true });
    }
  });
